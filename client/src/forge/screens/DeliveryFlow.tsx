...
import { ImportStoriesDialog } from "@/forge/components/ImportStoriesDialog";
...
export function DeliveryFlowScreen() {
  ...
  if (stories.length === 0 || !selectedStory) {
    return (
      <div className="flex flex-col items-center justify-center rounded-[20px] border border-dashed border-slate-200 bg-white px-8 py-24 text-center shadow-[0_20px_50px_-40px_rgba(15,23,42,0.22)]">
        <div className="rounded-2xl bg-slate-50 p-4 text-slate-300 ring-1 ring-slate-200">
          <Plus className="size-8" />
        </div>
        <h3 className="mt-6 text-xl font-semibold text-slate-950">No stories yet</h3>
        <p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">Create your first story to start the governed delivery flow. The Planning Agent will generate a brief, acceptance criteria, and architecture impact automatically.</p>
        <div className="mt-6 flex items-center gap-3">
          <Button
            onClick={() => setImportOpen(true)}
            variant="outline"
            className="flex items-center gap-2 rounded-full border-slate-200 text-slate-700 hover:bg-slate-50"
          >
        ...
        <ImportStoriesDialog open={importOpen} onOpenChange={setImportOpen} />
      </div>
    );
  }
  ...
  const hasRework = (selectedStory?.agentOutputs?.[selectedStory.phase]?.sections ?? [])
              ...
              <section className="rounded-[20px] border border-slate-200 bg-white p-5 shadow-[0_20px_50px_-40px_rgba(15,23,42,0.22)]">
                ...
                <LayoutGroup>
                  <div className="mt-5 flex gap-3 overflow-x-auto pb-2 snap-x snap-mandatory scrollbar-thin">
                    {grouped.map(group => (
                      <div key={group.phase} className="snap-start min-w-[180px] max-w-[260px] flex-1 rounded-[18px] border border-slate-200 bg-slate-50/60 p-3">
                        <div className="mb-3">
                          <div className="flex items-center justify-between">
                            <div>
                  ...
                  {selectedStory.phase !== "Operate" ? (
                    <>
                      <Button
                        ...
                        disabled={hasRework}
                        className="mt-4 flex w-full items-center justify-center gap-2 bg-slate-950 text-white hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed"
                      >
                      ...
                      {hasRework && (
                        <p className="mt-2 text-xs text-center text-amber-600">
                          Design rejected. Open the story drawer, go to Feedback tab, and click "Re-run Agent" before advancing.
                        </p>
                      )}
                    </>
                  ) : (
